...
def describe_game(hog, test_number, score0, score1, goal):
    ...
    end_of_turn = [(turn["s0_start"], turn["s1_start"])
                   for turn in game_trace[1:]]
...
def random_strat(seed):
    ...
    def random_strat(score, opponent_score):
        ...
        conditional_seed = score * 314159265358979 + opponent_score * \
            27182818284590452353602874713527 + seed * 161803398874989484820
...
def get_dice(seed):
    def dice():
        nonlocal seed
        seed, value = run_with_seed(seed, lambda: (
            random.randrange(0, 2**32), random.randrange(1, 7)))
        return value
    return dice
