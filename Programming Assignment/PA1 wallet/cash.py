...
class Banknote(Cash):
    ...
    def valid_bank(value: int) -> bool:
        """Check if the value is a multiple of 10 and a single digit.

        Arguments:
            value: the value of the banknote

        Returns:
            True if the value is a multiple of 10 and a single digit, False otherwise

        NOTE: It can do with recursion step too! but I prefer this solution
        """
        # check whether value is positive value
        if value < 9:
            return False

        value_original_str = str(value)
        # check condition
        first_must_be_non_zero = value_original_str[0] != '0'
        val_isdig = (value_str := value_original_str.replace('0', '')).isdigit()
        single_num_1_to_9 = len(value_str) == 1

        return val_isdig and first_must_be_non_zero and single_num_1_to_9
