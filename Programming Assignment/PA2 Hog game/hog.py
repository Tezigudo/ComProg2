...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        roll = dice()
        if roll == 1:
            flag = 1  # 1 is True on booleensystem
            continue
        _sum += roll
    return flag or _sum
...
def more_boar(player_score, opponent_score):
    ...
    False
    """
    player_score_str = str(player_score).zfill(2)[:2]
    opponent_score_str = str(opponent_score).zfill(2)[:2]
    return all(player_score_str[i] < opponent_score_str[i] for i in range(2))
    # return False
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1, running_high=running_high):
        ...
        if diff > running_high:
            print(
                f'Player {who} has reached a new maximum point gain. {diff} point(s)!')
            running_high = diff
...
def make_averaged(original_function, trials_count=10000):
    ...
    def avg(*args):
        total = sum(original_function(*args) for _ in range(trials_count))
        return total / trials_count

    return avg


def max_scoring_num_rolls(dice=six_sided, trials_count=100000):
    ...
    maximum = {'count': None, 'score': 0}
    avg = make_averaged(roll_dice, trials_count)
    for count in range(1, 11):
        result = {'count': count, 'score': avg(count, dice)}
        maximum = max(result, maximum, key=lambda x: x['score'])
...
def run_experiments():
    ...
    print('final_strategy average win rate:', average_win_rate(final_strategy))
...
def piggypoints_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    point = piggy_points(opponent_score)
    if point >= cutoff:
        return 0
    return num_rolls
...
def final_strategy(score, opponent_score, goal=GOAL_SCORE):
    """ This strategy is for modifying numrolls on more_bore_strategy.
    if difference between goal and score lessthan 17 numrolls will be 4.
    it will be 7 otherwise.
    """
    diff = goal - score
    num_roll = 4 if diff < 17 else 6
